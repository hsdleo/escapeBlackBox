...
void checkAlvo(){
    if(!abriuPassagem){
      ...
      if(abriuPassagem) {
        abrirPassagem();
        client.publish("topicoArduinoProntoA","2");
...
void checkQuadro(){
    if(!abriuSaida){
      ...
      if(abriuSaida){
        abrirSaida();
        client.publish("topicoArduinoProntoA","3");
        delay(300);
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("arduinoClientA")) {
      Serial.println("connected");
      desligarLed(ledConfig);
      ligarLed(ledOn);
      client.publish("topicoArduinoProntoA","1");
      client.subscribe("topicoPrincipalA");
...
void atualizaEEPROM(){
 // lê o valor na entrada analógica e guarda em val
  ...
  client.publish("contagem_alcatraz",buffer);
